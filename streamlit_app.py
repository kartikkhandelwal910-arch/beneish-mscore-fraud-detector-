...
@keyframes shine { from { background-position: -150% center; } to { background-position: 150% center; } }

.hero-banner h1 {
    background: linear-gradient(90deg, #ece4d1 25%, #c9a54a 50%, #ece4d1 75%);
    background-size: 200% auto;
    -webkit-background-clip: text; background-clip: text; color: transparent !important;
    animation: shine 6s ease-in-out infinite;
}
.hero-banner, [data-testid="stMetric"] { animation: fadeUp .6s ease forwards; }

.mscore-stamp {
    animation: stampPop .5s cubic-bezier(.2,1.4,.4,1) backwards;
    transition: transform .2s ease, box-shadow .2s ease;
}
.mscore-stamp:hover { transform: translateY(-2px) scale(1.03); box-shadow: 0 8px 18px -8px rgba(0,0,0,0.5); }
/* stagger the stamps so they pop in one after another instead of all at once */
.mscore-stamp:nth-of-type(1) { animation-delay: .05s; }
.mscore-stamp:nth-of-type(2) { animation-delay: .15s; }
.mscore-stamp:nth-of-type(3) { animation-delay: .25s; }
.mscore-stamp:nth-of-type(4) { animation-delay: .35s; }
.mscore-stamp:nth-of-type(5) { animation-delay: .45s; }
.mscore-stamp:nth-of-type(n+6) { animation-delay: .55s; }

.case-card { transition: transform .3s ease, border-color .3s ease, box-shadow .3s ease; animation: fadeUp .5s ease backwards; }
.case-card:hover { transform: translateY(-5px); border-color: #a3822f; box-shadow: 0 16px 32px -18px rgba(0,0,0,0.6); }

[data-testid="stDataFrame"] { animation: fadeUp .5s ease; }

.stTabs [data-baseweb="tab"] { transition: color .2s ease; }

@media (prefers-reduced-motion: reduce) {
    .hero-banner h1, .mscore-stamp, .case-card, [data-testid="stMetric"], [data-testid="stDataFrame"] {
